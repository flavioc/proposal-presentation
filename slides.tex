...
\alert[3]{new-distance(A, B1),}
\alert[3]{distance(A, B2),}
\alert[3]{B1 >= B2}
   \alert[3]{-o distance(A, B).}

\alert[4]{new-distance(A, B1),}
\alert[4]{new-distance(A, B2),}
\alert[4]{B1 <= B2}
   \alert[4]{-o new-distance(A, B1).}

\alert[5]{new-distance(A, D)}
   \alert[5]{-o \{B, W | !edge(A, B, W) |}
         \alert[5]{new-distance(B, D + W),}
         \alert[5,6]{set-priority(B, D + W))\},}
      \alert[5]{distance(A, D).}
         \end{verbatim}
      \end{block}
      \column{.45\textwidth}
      \begin{block}{Explanation}
         ...
         \begin{itemize}
            ...
            \item \texttt{distance/2} and \texttt{new-distance/2} represent the distance to \texttt{startnode}
            ...
            \only<3>{\item First rule deletes new distances that are longer than the current one}
