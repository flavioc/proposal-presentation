...
\alert[3]{new-distance(A, D2),}
\alert[3]{D1 <= D2}
   \alert[3]{-o new-distance(A, D1).}

\alert[4]{new-distance(A, D1),}
\alert[4]{shortest-distance(A, D2),}
\alert[4]{D1 >= D2}
   \alert[4]{-o shortest-distance(A, D2).}

\alert[5]{new-distance(A, D1),}
\alert[5]{shortest-distance(A, D2),}
\alert[5]{D1 < D2}
   \alert[5]{-o shortest-distance(A, D1),}
     \alert[5]{\{B, W | !edge(A, B, W) | }
         \alert[5]{new-distance(B, D1 + W),}
         \alert[5,7]{set-priority(B, D1 + W)\}.}
         \end{verbatim}
      \end{block}
      \column{.45\textwidth}
      \begin{block}{Explanation}
         ...
         \begin{itemize}
            ...
            \only<3,4>{\item First and second rule deletes longer distances}
            \only<5>{\item Third rule replaces current shortest distance and propagates the new distance to the neighborhood}
