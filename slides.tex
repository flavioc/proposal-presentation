     ...
     \alert[4]{\{B, W | !edge(A, B, W) | }
      ...
      \begin{block}{Explanation}
         ...
         \begin{itemize}
            ...
            \only<3>{\item First rule deletes longer distances}
            \only<4>{\item Second rule replaces current shortest distance and propagates the new distance to the neighborhood}
            \only<5>{\item Node priorities are in ascending order (smaller is better)}
